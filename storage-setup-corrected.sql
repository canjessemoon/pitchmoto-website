-- Comprehensive Storage Setup for PitchMoto (CORRECTED VERSION)
-- Run this in Supabase SQL Editor to create buckets and secure policies
-- This version uses 'user_profiles' table (not 'profiles')

-- IMPORTANT: This script matches the bucket names in storage-helpers.ts AND your database schema
...
-- Investors can view pitch decks (CORRECTED: uses user_profiles table)
CREATE POLICY "Investors can view pitch decks" ON storage.objects 
FOR SELECT USING (
  bucket_id = 'pitch-decks' AND
  EXISTS (SELECT 1 FROM user_profiles WHERE user_id = auth.uid() AND user_type IN ('investor', 'admin'))
);
...
-- Investors can view pitch videos (CORRECTED: uses user_profiles table)
...
SELECT 'Comprehensive storage setup completed successfully!' as status,
       ...
       'All security policies applied using user_profiles table' as security;